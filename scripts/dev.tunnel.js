        ...
        exec(`curl -s ${NGROK_API_URL}`, (error, stdout) => {
          if (error) {
            if (attempt < maxAttempts) {
              console.log(`   Waiting for ngrok to start (attempt ${attempt}/${maxAttempts})...`);
              pollNgrokUrl(attempt + 1, maxAttempts);
              return;
            }
            console.error('❌ Failed to get ngrok URL after multiple attempts:', error.message);
            console.error('   Make sure ngrok is running and accessible at localhost:4040');
            reject(error);
            return;
          }
          ...
          try {
            ...
            if (httpsTunnel) {
              ...
              console.log(`   No HTTPS tunnel found yet (attempt ${attempt}/${maxAttempts})...`);
              pollNgrokUrl(attempt + 1, maxAttempts);
            } else {
              const error = new Error('No HTTPS tunnel found after multiple attempts');
              console.error('❌', error.message);
              reject(error);
            }
          } catch (parseError) {
            if (attempt < maxAttempts) {
              console.log(`   Invalid response from ngrok API (attempt ${attempt}/${maxAttempts})...`);
              pollNgrokUrl(attempt + 1, maxAttempts);
            } else {
              console.error('❌ Failed to parse ngrok response after multiple attempts:', parseError.message);
              reject(parseError);
            }
          }
        });
...
async function startSlackApp() {
  return new Promise((resolve, reject) => {
    ...
    slackProcess.on('error', (error) => {
      ...
      console.error('   Make sure Slack CLI is installed: https://api.slack.com/automation/cli/install');
      reject(error);
    });
    ...
    slackProcess.on('exit', (code, signal) => {
